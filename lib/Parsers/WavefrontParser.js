import { Resource, ResourceType } from '../Core';
import { VertexAttribute } from '../GL/Helpers';
import DataType from '../GL/Helpers/DataType';
import Exception from '../Core/Exception';
import { Vector2, Vector3 } from '../Math';
import Vector4 from '../Math/Vector4';
var Token;
(function (Token) {
    Token["Vertex"] = "v";
    Token["Normal"] = "vn";
    Token["Texcoord"] = "vt";
    Token["Face"] = "f";
})(Token || (Token = {}));
export class Vertex {
    constructor(vertex, normal, texcoord) {
        this.vertex = new VertexAttribute(vertex, DataType.Float32, false);
        this.normal = new VertexAttribute(normal, DataType.Int8, true);
        this.texcoord = new VertexAttribute(texcoord, DataType.Int16, false);
    }
}
class WavefrontParser {
    constructor() {
        this.raw = {
            [Token.Vertex]: [],
            [Token.Normal]: [],
            [Token.Texcoord]: [],
        };
        this.vertices = [];
        this.indices = [];
        this.hash = {}; // @todo Make this (and possibly others) `HashSet`-like
        this.index = 0;
    }
    async load(path) {
        let reader;
        try {
            reader = await Resource.load(path, ResourceType.OBJ);
        }
        catch (e) {
            throw new Exception(`WavefrontParser: failed to load '${path}'`, e);
        }
        for (let line of reader) {
            if (line[0] === '#') {
                continue;
            }
            const parts = line.split(' ');
            const token = parts.shift();
            switch (token) {
                case Token.Vertex:
                case Token.Normal:
                case Token.Texcoord:
                    this.raw[token].push(...parts.map((part) => +part));
                    break;
                case Token.Face:
                    this.parse(parts);
                    break;
            }
        }
    }
    parse(parts) {
        for (let i = 0; i < 3; i++) {
            const face = parts[i];
            if (face in this.hash) {
                this.indices.push(this.hash[face]);
                continue;
            }
            const x = face.split('/').map((v) => ((+v) - 1));
            // noinspection PointlessArithmeticExpressionJS
            const vertex = new Vector3(this.raw[Token.Vertex][(x[0] * 3) + 0], this.raw[Token.Vertex][(x[0] * 3) + 1], this.raw[Token.Vertex][(x[0] * 3) + 2]);
            // noinspection PointlessArithmeticExpressionJS
            const normal = new Vector4(this.raw[Token.Normal][(x[2] * 3) + 0], this.raw[Token.Normal][(x[2] * 3) + 1], this.raw[Token.Normal][(x[2] * 3) + 2], 0);
            // noinspection PointlessArithmeticExpressionJS
            const texcoord = new Vector2(this.raw[Token.Texcoord][(x[1] * 2) + 0], this.raw[Token.Texcoord][(x[1] * 2) + 1]);
            this.hash[face] = this.index;
            this.vertices.push(new Vertex(vertex, normal, texcoord));
            this.indices.push(this.index);
            this.index += 1;
        }
    }
}
export default WavefrontParser;
