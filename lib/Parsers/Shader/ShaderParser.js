    ...
    parseCapability(line) {
        ...
        switch (cap) {
            case ShaderCapability.Blend: {
                const [srcRGB, dstRGB, srcA, dstA] = params;
                if (srcRGB === ShaderBlendFunction.Off) {
                    this.capabilities[ShaderCapability.Blend] = false;
                }
                else {
                    this.capabilities[ShaderCapability.Blend] = {
                        srcRGB: blendFunction(srcRGB),
                        dstRGB: blendFunction(dstRGB),
                        srcA: srcA ? blendFunction(srcA) : false,
                        dstA: dstA ? blendFunction(dstA) : false,
                    };
                }
                break;
            }
