...
class CubeMap extends Texture {
    ...
    static async load(url) {
        const name = Path.getFileName(url);
        let dds;
        try {
    ...
    apply() {
        ...
        for (let face = 0; face < 6; face++) {
            ...
            for (let level = 0; level < this.mipmapCount; level++) {
                ...
                Context.texImage2D(Context.TEXTURE_CUBE_MAP_POSITIVE_X + face, level, this.pixelFormat, mipmap.width, mipmap.height, 0, this.pixelFormat, this.pixelType, mipmap.data);
            }
        }
    }
}
