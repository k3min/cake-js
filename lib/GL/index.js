...
class GL {
    constructor() {
        ...
        this.context = document.createElement('canvas').getContext('webgl');
        this.ext = this.context.getExtension('WEBGL_draw_buffers');
        this.depth = this.context.getExtension('WEBGL_depth_texture');
    }
    static get export() {
        const { context: { enable: enableRaw, disable: disableRaw, ...context }, getExtensions, enable, disable, ext, depth, } = new GL();
        return {
            ...context,
            enableRaw,
            disableRaw,
